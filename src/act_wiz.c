...
void do_mstat( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC( victim ) )
      pager_printf_color( ch, "&cMob hitdie : &C%dd%d+%d    &cMob damdie : &C%dd%d+%3d    &cIndex damdie : &C%dd%d+%3d\n\r&cNumAttacks : &C%d\n\r",
   ...
   for( d = first_descriptor; d; d = d->next )
    {
	 ...
	 case CON_GET_OLD_PASSWORD:		st = "Get password"; break; 
	 case CON_CONFIRM_NEW_NAME:		st = "Confirm name"; break;
	 case CON_GET_NEW_PASSWORD:		st = "New password"; break;
	 case CON_CONFIRM_NEW_PASSWORD:	st = "Confirm password"; break;
	 case CON_GET_NEW_SEX:			st = "Get sex"; break;
	 case CON_READ_MOTD:			st = "Reading MOTD"; break;
	 case CON_EDITING:			st = "In line editor"; break;
	 case CON_PRESS_ENTER:			st = "Press enter"; break;
 	 default:					st = "Invalid!!!!"; break;
     }
...
void do_form_password( CHAR_DATA *ch, char *argument ) 
{
   ...
   {
      send_to_char( "Usage: formpass <password>\n\r", ch );
      send_to_char( "New password must be atlest 5 characters in length.\n\r", ch );
      return;
   }
