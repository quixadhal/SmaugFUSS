 ...
 * Sort them by level - Added on 9/11/2005
 */
void do_hlist( CHAR_DATA * ch, char *argument )
{
   int min, max, minlimit, maxlimit, cnt, count, level, col;
   char arg[MAX_INPUT_LENGTH];
   HELP_DATA *help;
   ...
   if( minfound && !maxfound )   /* So they can view just one level if they want to */
      max = URANGE( minlimit, min, maxlimit );
   ...
   for( level = min; level <= max; level++ )
   {
      ...
      {
         if( help->level == level && ( !idx || nifty_is_name_prefix( idx, help->keyword ) ) )
         {
            count++;
            if( count == 1 )
               pager_printf( ch, "\n\r[Level %3d]\n\r", level );
